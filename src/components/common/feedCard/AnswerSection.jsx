...
export default function AnswerSection({ page }) {
  const [inputValue, setInputValue] = useState({});
  const src = '/public/assets/sampleProfile.png';

  const onChangeInput = inputState => {
    setInputValue(prev => ({ ...prev, ...inputState }));
  };

  return (
    <Container>
      <UserProfileImg size="48px" alt="userProfile" src={src} />
      <FlexColumn>
        <FlexRow>
          <UserName userName="아초는 고양이" size="18px" />
          {page === 'questionFeed' && <DatesAgo text="2주전" />}
        </FlexRow>
        {page === 'questionFeed' && <AnswerContent content={content} />}
        {page === 'answer' && (
          <InputTextarea
            height="18.6rem"
            marginbottom="0.8rem"
